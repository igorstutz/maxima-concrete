...
// Para onde o lead vai depois de enviar. A página explica o que acontece
// agora, quando ligamos e de quais números — informação que não cabia no aviso
// de uma linha que ficava no lugar do formulário.
...
export default function Contact({ content }: { content: Record<string, any> }) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSelectedOption("");
      setSelectedServices([]);
      // Navegação pelo roteador, não window.location: a página não é
      // descarregada, então os beacons que pushLeadEvent acabou de enfileirar
      // ainda saem. Um reload aqui cancelaria a medição da conversão.
  ...
  return (
    <section id="contact" className="scroll-mt-20 bg-[#EAEAEA] py-16 sm:py-24">
      <Container>
        ...
        {status === "success" ? (
          // O pedido entrou e o navegador já está a caminho de /thank-you/.
          // Isto aparece pelo instante que a troca de página leva — nunca o
          // formulário vazio de novo, que passaria a ideia de que nada
          // aconteceu e convidaria a um segundo envio.
          <div
            role="status"
            aria-live="polite"
          ...
          >
            ...
            <p className="flex items-center justify-center gap-2 text-navy/70">
              <Loader2 className="h-4 w-4 animate-spin" />
              Got it — taking you to the next steps…
            </p>
          </div>
        ) : (
